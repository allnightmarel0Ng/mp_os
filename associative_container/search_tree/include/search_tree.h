...
public:
    ...
    public:
        ...
        explicit common_node(typename associative_container<tkey, tvalue>::key_value_pair *keys_and_values, common_node **subtrees, size_t t);
...
protected:
    ...
    explicit search_tree(std::function<int(tkey const &, tkey const &)> keys_comparer = std::less<tkey>(), logger *logger = nullptr, allocator *allocator = nullptr);
    
public:
    
    virtual std::vector<typename associative_container<tkey, tvalue>::key_value_pair> obtain_between(tkey const &lower_bound, 
        tkey const &upper_bound, bool lower_bound_inclusive, bool upper_bound_inclusive) = 0;
...
template<typename tkey, typename tvalue>
search_tree<tkey, tvalue>::common_node::common_node(typename associative_container<tkey, tvalue>::key_value_pair *keys_and_values, common_node **subtrees, size_t t):
    keys_and_values(keys_and_values), subtrees(subtrees), virtual_size(0)
{
...
template<typename tkey, typename tvalue>
int search_tree<tkey, tvalue>::node_find_path(typename search_tree<tkey, tvalue>::common_node const *node, tkey const &key, size_t left_bound_inclusive, size_t right_bound_inclusive)
{
    int index;
    while (true)
    {
...
template<typename tkey, typename tvalue>
void search_tree<tkey, tvalue>::node_insert(typename search_tree<tkey, tvalue>::common_node *node, typename associative_container<tkey, tvalue>::key_value_pair &&kvp, size_t subtree_index, typename search_tree<tkey, tvalue>::common_node *right_subtree)
{
...
template<typename tkey, typename tvalue>
std::pair<typename search_tree<tkey, tvalue>::common_node *, typename associative_container<tkey, tvalue>::key_value_pair> search_tree<tkey, tvalue>::node_split(typename search_tree<tkey, tvalue>::common_node *node, typename associative_container<tkey, tvalue>::key_value_pair &&kvp, size_t subtree_index, typename search_tree<tkey, tvalue>::common_node *right_subtree)
{
...
template<typename tkey, typename tvalue>
void search_tree<tkey, tvalue>::merge_nodes(typename search_tree<tkey, tvalue>::common_node *parent, size_t left_subtree_index)
{
...
template<typename tkey, typename tvalue>
search_tree<tkey, tvalue>::search_tree(std::function<int(tkey const &, tkey const &)> keys_comparer, logger *logger, allocator *allocator): 
    _keys_comparer(keys_comparer), _logger(logger), _allocator(allocator), _root(nullptr)
{
