    ...
    _denominator(std::forward<big_integer>(denominator))
{
    ...
    {
        throw std::invalid_argument("Attempt to create a fraction with denominator = 0");
    }
    normalize_fraction();
}
...
fraction::fraction(fraction const &other) = default;

fraction &fraction::operator=(fraction const &other) = default;

fraction::fraction(fraction &&other) noexcept = default;

fraction &fraction::operator=(fraction &&other) noexcept = default;
...
std::ostream &operator<<(std::ostream &stream, fraction const &obj)
{
    ...
    if (obj._denominator != big_integer("1"))
    {
...
std::istream &operator>>(std::istream &stream, fraction &obj)
{
    ...
    auto denominator = has_denominator
        ? value.substr(pos + 1)
        : "1";
...
fraction fraction::cos(fraction const &epsilon) const
{
    if (fraction::abs(*this) > fraction(big_integer("1"), big_integer("1")))
    {
        throw std::invalid_argument("To compute cos fraction must be between [-1, 1]");
    }
...
fraction fraction::sec(fraction const &epsilon) const
{
    ...
    return fraction(cosine._denominator, cosine._numerator);
}

fraction fraction::cosec(fraction const &epsilon) const
{
    fraction sine = sin(epsilon);
    return fraction(sine._denominator, sine._numerator);
}
